...
class HourglassModel():
	...
	def generate_model(self):
		...
		with tf.device(self.gpu):
			...
			# self.output_sig = tf.nn.sigmoid(self.output, name='output_sigmoid')
			graphTime = time.time()
		...
		with tf.device(self.cpu):
			...
			with tf.name_scope('summary'):
				for i in range(len(self.accIdx)):
					tf.summary.scalar('joint_{}'.format(i), self.joint_accur[i], collections = ['test'])
	...
	def visualize_valid(self, imgi, hm, set, name=None):
		...
		if set == 'valid':
			out = self.Session.run(self.output_sig, feed_dict = {self.img : inp})
			outhm = out[0,-1]
		elif set == 'train':
			outhm = hm[-1]
	...
	def _train(self, nEpochs = 10, epochSize = 1000, saveStep = 500, validIter = 10):
		...
		with tf.name_scope('Train'):
			...
			for epoch in range(nEpochs):
				...
				num_show = 10 if self.batchSize > 10 else self.batchSize
				for i in range(num_show):
					# jts_t = self.visualize_valid(img_valid[i], gt_valid[i], 'train', os.path.join(self.img_save_dir, 'label_{}.jpg'.format(i)))
					jts_v = self.visualize_valid(img_valid[i], gt_valid[i], 'valid', os.path.join(self.img_save_dir, 'infer_{}.jpg'.format(i)))
	...
	def record_valid_diff(self, jts_v, jts_t):
		save_file_name = os.path.join(self.img_save_dir, 'diff.txt')
		ofile = open(save_file_name, 'w')
		delta = (jts_v - jts_t)/256
		delta.astype(np.float32)
		for i in range(jts_v.shape[0]):
			diff = np.sqrt( np.sum(np.multiply(delta[i], delta[i])) )
			ofile.write(str(i)+': ')
			ofile.write(str(round(diff, 4))+'\n')
		ofile.close()
	...
	def _argmax(self, tensor):
		...
		return (tf.to_float(argmax % self.output_res), tf.to_float(argmax // self.output_res))

	def _posmean(self, tensor):
		win = tensor.get_shape().as_list()[0]
		index = tf.constant(np.arange(win), shape=[win], dtype=tf.float32)
		pos_x = tf.reduce_sum(tf.multiply(tf.reduce_sum(tensor, axis=0), index))
		pos_y = tf.reduce_sum(tf.multiply(tf.reduce_sum(tensor, axis=1), index))
		sum_all = tf.reduce_sum(tensor)
		return (tf.divide(pos_x,sum_all), tf.divide(pos_y,sum_all))
	...
	def _compute_err(self, u, v):
		...
		if 1:
			...
			u_x,u_y = self._posmean(u)
			v_x,v_y = self._argmax(v)
		return tf.divide(tf.sqrt(tf.square(u_x - v_x) + tf.square(u_y - v_y)), tf.to_float(self.output_res))
	...
	def _accur(self, pred, gtMap, num_image):
		...
		Returns:
			(float)
		"""
		pred_sig = tf.sigmoid(pred)
		err = tf.to_float(0)
		for i in range(num_image):
			err = tf.add(err, self._compute_err(pred_sig[i], gtMap[i]))
	...
	def _distAccuracy(self, pred, gtMap, num_image):
		...
		# index = tf.constant(np.arange(w), shape=[w, 1], dtype=tf.float32)
		# pred = tf.sigmoid(pred)
		# pred_row = tf.reduce_sum(pred, axis=1)
		# pred_col = tf.reduce_sum(pred, axis=2)
		# pred_sum = tf.reshape(tf.reduce_sum(pred, axis=[1,2]), [num_image, -1])
		# preds_row = tf.divide(tf.matmul(pred_row, index), pred_sum)
		# preds_col = tf.divide(tf.matmul(pred_col, index), pred_sum)
		# preds = tf.stack([tf.reshape(preds_row, [-1]), tf.reshape(preds_col, [-1])], axis=0)
	...
	def _graph_hourglass_mobile(self, inputs):
		...
		with tf.name_scope('model'):
			...
			with tf.name_scope('preprocessing'):
				...
				# r2 = self._residual_mobile(pool1, numOut=self.nFeat, multi = self.multi, name = 'r2')
				# pool2 = tf.contrib.layers.max_pool2d(r2, [2,2], [2,2], padding='VALID')
				# r3 = self._residual_mobile(pool2, numOut=self.nFeat, multi=self.multi, name='r3')

				r3 = self._residual_mobile(pool1, numOut= self.nFeat, multi = self.multi, name = 'r3')
				sum_[0] = r3
			...
			with tf.name_scope('stacks'):
				for i in range(self.nStack):
					with tf.name_scope('stage_' + str(i)):
						...
						ll[i] = self._conv_bn_relu(hgr, self.nFeat, 1, 1, 'VALID', name= 'conv')				
						# out[i] = self._conv(ll[i], self.outDim, 1, 1, 'VALID', 'out')
						out[i] = self._deconv(ll[0], self.outDim, 3, 2, 'SAME', 'out')
						if i < self.nStack - 1:
							ll_[i] = self._conv(ll[i], self.nFeat, 1, 1, 'VALID', 'll')
							# out_[i] = self._conv(out[i], self.nFeat, 1, 1, 'VALID', 'out_')
	...
	def _residual_mobile(self, inputs, numOut, multi=6, name = 'residual_block'):
		...
		with tf.name_scope(name):
			...
			return tf.add_n([convb, skipl], name = 'res_block')
		# with tf.name_scope(name):
		# 	pad = tf.pad(inputs, np.array([[0,0], [1,1], [1,1], [0,0]]), name = 'pad')
		# 	conv_d = self._conv_depthwise(pad, kernel_size=3, strides=1, pad = 'VALID', name = 'conv_d')
		# 	norm_1 = tf.contrib.layers.batch_norm(conv_d, 0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training)
		# 	conv   = self._conv(norm_1, numOut, kernel_size=1, strides=1, pad='VALID', name='conv')
		# 	norm_2 = tf.contrib.layers.batch_norm(conv, 0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training)
		# 	# skipl = self._skip_layer(inputs, numOut)
		# 	# return tf.add_n([norm_2, skipl], name='res_block')
		# 	return norm_2
